...
// Khởi tạo editMode và items
const message = useMessage();

const editMode = ref(false);
const items = ref([
  { id: 1, ts: 300, label: "Giảng viên" },
  { id: 2, ts: 20000, label: "Học viên" },
  { id: 3, ts: 10000, label: "Bài giảng" },
]);

// Tạo editData dựa trên dữ liệu ban đầu của items
const editData = ref([
  { label: "Tên giảng viên", value: items.value[0].label, type: "text" },
  { label: "Số giảng viên", value: items.value[0].ts, type: "text" },
  { label: "Tên học viên", value: items.value[1].label, type: "text" },
  { label: "Số học viên", value: items.value[1].ts, type: "text" },
  { label: "Tên bài giảng", value: items.value[2].label, type: "text" },
  { label: "Số bài giảng", value: items.value[2].ts, type: "text" },
]);

// Hàm lưu chỉnh sửa
const saveChanges = () => {
  editMode.value = false; // Ẩn form chỉnh sửa
  // Cập nhật dữ liệu từ editData vào items
  items.value[0].label = editData.value[0].value;
  items.value[0].ts = editData.value[1].value;
  items.value[1].label = editData.value[2].value;
  items.value[1].ts = editData.value[3].value;
  items.value[2].label = editData.value[4].value;
  items.value[2].ts = editData.value[5].value;
...
const cancelEdit = () => {
  ...
  editData.value[1].value = items.value[0].ts;
  editData.value[2].value = items.value[1].label;
  editData.value[3].value = items.value[1].ts;
  editData.value[4].value = items.value[2].label;
  editData.value[5].value = items.value[2].ts;
};
