<template>
  <div class="w-full mb-[100px] m-auto">
    ...
    <div class="w-full flex">
      ...
      <!-- <n-button @click="editMode = true"> Sửa </n-button> -->
...
const saveChanges = () => {
  editMode.value = false; // Thoát khỏi chế độ chỉnh sửa
  // Lưu các thay đổi vào items
  title.value = editData.value.editTitle;
  subTitle.value = editData.value.editSubTitle;
  items.value.forEach((item, index) => {
    item.nameTitle = editData.value[`editNameTitle${index + 1}`]; // Lấy giá trị từ editData
    item.description = editData.value[`editDescription${index + 1}`]; // Lấy giá trị từ editData
    item.imageSrc = editData.value[`editImageSrc${index + 1}`]; // Lấy giá trị từ editData
  });
...
const cancelEdit = () => {
  ...
  items.value.forEach((item, index) => {
    editData.value[`editNameTitle${index + 1}`] = item.nameTitle; // Lấy giá trị từ editData
    editData.value[`editDescription${index + 1}`] = item.description; // Lấy giá trị từ editData
    editData.value[`editImageSrc${index + 1}`] = item.imageSrc; // Lấy giá trị từ editData
  });
};
</script>
